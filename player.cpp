...
private:
  ...
  void getArguments(int argc, const char **argv) {
    ...
    rPort = getPort(argv[3]);
    filename = std::string(argv[4]);
    mPort = getPort(argv[5]);
  ...
  bool checkSocket(epoll_event &event, Socket sock, bool header) {
  ...
  Socket connect() {
    ...
    _getaddrinfo(host.c_str(), std::to_string(rPort).c_str(), &hints, &result);
